...
const CareerAllAnalysis: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      ...
      return matchesSearch && matchesDept && matchesBatch && matchesPi && matchesIncharge && matchesPlaced;
    });
  }, [rows, search, dept, batch, pi, incharge, placed]);
  ...
  const metrics = useMemo(() => {
    ...
    const totalNotInBatch = filtered.reduce((s, r) => s + toNumber(r['Not in Batch']), 0);
    ...
    filtered.forEach(r => { deptPackagePoints.push({ dept: String(r['Dept'] || 'NA'), x: Math.max(0, deptPackagePoints.length+1), y: toNumber(r['Package']) }); });
    ...
    filtered.forEach(r => { const inc = String(r['Incharge'] || 'NA'); inchargeMap.set(inc, (inchargeMap.get(inc)||0) + toNumber(r['No Of Student Placed'])); });
    ...
    filtered.forEach(r => { const p = String(r['PI'] || 'NA'); const cur = piMap.get(p) || { percentSum: 0, count: 0 }; cur.percentSum += toNumber(r['Percentage']); cur.count++; piMap.set(p, cur); });
